...
def rich_employee_info(employee_list):
    ...
    table = Table(title="[bold green]Employee Information[/bold green]", box=box.ROUNDED)
...
def rich_package_selection():
    ...
    table.add_row("Regular", "General-Tidying, Sweep, Dust, Mop", "$100.00")
    table.add_row("Premium", "Regular Service+, Bathrooms, Closets, Laundry", "$200.00")
    table.add_row("Outdoor", "Mowing, Weed-Wack, Shrubs, Leaves", "$300.00")
...
def rich_customer_daily_summary(customers_data):
    ...
    table = Table(title="[bold blue]Today's Customer Summary[/bold blue]", box=box.DOUBLE)
